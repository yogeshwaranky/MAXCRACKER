        ...
        <table className="table table-striped table-bordered table-hover">
          ...
          <tbody>
            {currentItems.map((items, index) => (
              <tr key={index}>
                <td><img src={items.image} alt="Cracker Item" style={{ width: '100px' }} /></td>
                ...
                <td>
                  <input 
                    type="number" 
                    value={1} // Using fixed 1 for quantity in this case
                    style={{ width: '80px' }} 
                    min="1" 
                  />
                </td>
                <td>{(1 * items.finalPrice).toFixed(2)}</td> {/* Assuming quantity is 1 */}
              </tr>
            ))}
          </tbody>
        </table>
